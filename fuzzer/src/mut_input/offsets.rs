...
// Merges two operands' taint offsets into one combined region (e.g. both sides of `x == y`
// when x and y are both tainted) -- used by search/reusing.rs to also try the reuse pool's
// pattern for "both operands together", not just each one independently.
